...
export default function VideogamePage() {
    ...
    const { videogame, fetchVideogame, fileUrl, isCoverOverlayOpen, isScreenshotOverlayOpen, currentIndex, handleCoverClick, handleCoverOverlayClick, handleScreenshotClick, handleScreenshotOverlayClick, goToPrevSlide, goToNextSlide } = useGlobalContext();
    ...
    return (
       ...
       {isCoverOverlayOpen && <CoverOverlay title={videogame.name} src={fileUrl + videogame.cover} alt={videogame.name} handleCoverOverlayClick={handleCoverOverlayClick}/>}

       {isScreenshotOverlayOpen && <ScreenshotOverlay title={videogame.name} src={fileUrl + videogame.screenshots[currentIndex].url} alt={videogame.name} handleScreenshotOverlayClick={handleScreenshotOverlayClick} prev={goToPrevSlide} next={goToNextSlide} currentIndex={currentIndex + 1} length={videogame.screenshots.length}/>}
            ...
            <div className="row gy-4">
                ...
                <div className="col-12 col-lg-6">
                    ...
                    <img src={fileUrl + videogame.cover}
                            ...
                            style={{cursor:"zoom-in", objectFit:"contain"}} onClick={handleCoverClick}/>
                    </div>
                    ...
                    {videogame.screenshots?.map((screenshot, index) => {
                        ...
                        <img src={fileUrl + screenshot.url} alt={ videogame.name }
                            className="current-screenshot" style={{cursor:"zoom-in"}} onClick={() => handleScreenshotClick(index)}/>
                    </div>
                    )
                    })}
                    </div>
