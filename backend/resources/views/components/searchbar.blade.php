<form
    method="GET"
    action="{{ $route }}"
    class="input-group d-flex flex-column flex-lg-row gap-3 mb-3"
>
    <div class="form-outline position-relative">
        <input
            ...
            id="searchInput"
            name="search"
            class="form-control pr-5 bg-white pe-4"
            value="{{ request("search") }}"
            placeholder="Cerca per {{ $subject }}"
            oninput="toggleClearButton('searchInput')"
        />
        <span
            id="clear-btn-searchInput"
            class="position-absolute mt-2 top-0 end-0 pe-2"
    ...
    <div class="d-flex align-items-start gap-3">
        <select name="orderFor" id="orderFor" class="form-select bg-white">
            <option
                value="create"
                {{ request("orderFor") == "create" ? "selected" : "" }}
            >
                Data creazione
            </option>
            ...
            <option
                value="edit"
                {{ request("orderFor") == "edit" ? "selected" : "" }}
            >
