...
<div id="icons"
    ...
    <a class="text-decoration-none text-dark fs-4 fs-md-2" href="{{ route('admin.videogames.edit', $videogame) }}">
        <i id="show-pencil" class="bi bi-pencil"></i>
    </a>
    <button type="button" class="text-decoration-none text-dark btn p-0 fs-4 fs-md-2" data-bs-toggle="modal"
        data-bs-target="#deleteModal" data-videogame-slug="{{ $videogame->slug }}"
        data-videogame-name="{{ $videogame->name }}">
        <i id="show-trash" class="bi bi-trash"></i>
    </button>
</div>
